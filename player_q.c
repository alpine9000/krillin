...
player_q_get_input(player_t* player)
{
  ...
  if (player->first_run) {
	...
	for (int i = 0; i < player->replay_batch_size; i++) {
	  ...
	  for (int a = 0; a < ACTION_NUM_ACTIONS; a++) {
	    ...
	    q_table_row[a] =  player->q_model->run(player->q_model, &next_state)[0];
	    //printf("%f ", q_table_row[a]);
	  }

	  number_t updated_q[1];
	  updated_q[0] = batch[i].reward + (player->discount * misc_q_table_row_max(q_table_row, ACTION_NUM_ACTIONS));
	...
	player->q_model->train(player->q_model,  player->train, 5);
      }
    }
  }
  ...
  for (int a = 0; a < ACTION_NUM_ACTIONS; a++) {
    ...
    // Run the network for this action and get q table row entry
#ifdef GAME_TARGET_MODEL
    q_table_row[a] = player->q_target_model->run(player->q_target_model, &state_action)[0];
#else
    q_table_row[a] =  player->q_model->run(player->q_model, &state_action)[0];
#endif
  }
