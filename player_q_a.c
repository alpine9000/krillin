...
player_q_get_input(player_t* player)
{
    ...
    *rmp = player->state;
	...
	for (int qi = 0; qi < GAME_NUM_OUTPUTS; qi++) {
	  ...
	  if (qi ==  batch[i].previous_action) {
	    const number_t *q_table_row = player->q_nn_model->run(player->q_nn_model, batch_state.state);

	    number_t maxq = batch[i].reward + (player->discount * misc_q_table_row_max(q_table_row, 0));
	  ...
	  player->q_nn_model->set_training_output_data(player->train, i, qi, updated_q);
	}
      }
  ...
  number_t rr = frand();
  const number_t* q_table_row = player->q_nn_model->run(player->q_nn_model, next_state.state);
